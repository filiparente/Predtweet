...
def load_inputids(path = "bitcoin_data/", batch_size=1000, window_size=3, discretization_unit=1, approx=((3600*10)/15)):
    ...
    dev_dataloader = DataLoader(dev_dataset, sampler=SequentialSampler(dev_dataset), batch_size=batch_size, num_workers=4)
    test_dataloader = DataLoader(test_dataset, sampler=SequentialSampler(test_dataset), batch_size=batch_size, num_workers=4)
