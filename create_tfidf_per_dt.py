...
def ChunkIterator(df, cleaning, n_chunks, chunksize, n_tot_sent, n_en_sent, train_split_date, dev_split_date, test_split_date, args):
            ...
            for tweet in range((n_chunks-1)*chunksize, nRow+(n_chunks-1)*chunksize, 1):
                ...
                if tweet_time>next_date: 
                    ...
                    if n<train_split_date:#tweet_time<train_split_date:
                        ...
                        test_tweet_times.extend(tweet_times)
                    n+=1
    ...
    assert sum([len(t) for t in train_tweets]) == len(train_tweet_times)
    assert sum([len(t) for t in dev_tweets]) == len(dev_tweet_times)
    assert sum([len(t) for t in test_tweets]) == len(test_tweet_times)
    ...
    if args.create:
        ...
        for f in result:
            with open(f, "r") as infile:
                ...
                n += 1
                infile.close()
        #start_date = pd.to_datetime(1556668801000000000)
        #end_date = pd.to_datetime(1557516579000000000)
