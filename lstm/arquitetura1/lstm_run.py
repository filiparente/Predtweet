...
def load_input(path = "bitcoin_data/", window_size=3, discretization_unit=1):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=SequentialSampler(train_dataset), batch_size=24, num_workers=1, collate_fn=collate_fn)
    dev_dataloader = DataLoader(dev_dataset, sampler=SequentialSampler(dev_dataset), batch_size=1, num_workers=1, collate_fn=collate_fn)
    test_dataloader = DataLoader(test_dataset, sampler=SequentialSampler(test_dataset), batch_size=1, num_workers=1, collate_fn=collate_fn)
