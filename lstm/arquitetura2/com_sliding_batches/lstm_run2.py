...
# TODO: pôr sliding batches a funcionar; prever apenas 1 contagem; sem sliding batches corrigir hidden no test
...
class DatasetSlidingBatches(Dataset):
    def __init__(self, dataset, len_):
...
def main():
    ...
    for i in range(max_len_train):
        ...
        trainX.append(train_X[:,i])
        trainy.append(train_obs_seq[i])
    ...
    for i in range(max_len_test):
        testX.append(test_X[:,i])
        testy.append(test_obs_seq[i])
    ...
    if args.use_features:
        ...
        train_dataset = create_dataset2(train_obs_seq, seq_len)
        dev_dataset = create_dataset2(np.concatenate((train_obs_seq[-seq_len:],dev_obs_seq)), seq_len)
        test_dataset = create_dataset2(np.concatenate((dev_obs_seq[-seq_len:],test_obs_seq)), seq_len)

        assert len(train_dataset[0])>0, "Batch size or sequence length too big for training set with only " + str(max_len_train) +" samples"
        assert len(dev_dataset[0])>0, "Batch size or sequence length too big for validation set with only " + str(max_len_dev) +" samples"
        assert len(test_dataset[0])>0, "Batch size or sequence length too big for test set with only " + str(max_len_test) +" samples"
    ...
    train_dataset2 = DatasetSlidingBatches(train_dataset_, len(train_dataset[0]))
    ...
    decoder = Decoder(HIDDEN_DIM, device, args.use_features)
    ...
    # Load best model
    if args.test_acc or args.evaluate_only:
