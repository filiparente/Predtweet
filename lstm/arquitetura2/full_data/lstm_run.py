...
def evaluate(args, encoder, decoder, eval_X, eval_y, criterion, device, global_step, epoch, prefix="", store=True):
    #pdb.set_trace()
    # Validation
    ...
    trainX_sample = eval_X
    trainY_sample = eval_y
    ...
    trainX_sample = trainX_sample.reshape(1, len(eval_y), 768)
    trainY_sample = trainY_sample.reshape(len(eval_y),1)
...
def main():
    ...
    #Create model
    encoder = Encoder(EMBEDDING_DIM, HIDDEN_DIM, len(train_obs_seq), device)
    ...
    for epoch in train_iterator:
        ...
        trainX_sample = train_X
        trainY_sample = train_obs_seq
        ...
        # Do forward pass through encoder: get hidden state
        #hidden = encoder(trainX_sample)    
        trainX_sample = trainX_sample.reshape(1,len(train_obs_seq),EMBEDDING_DIM)
        trainY_sample = trainY_sample.reshape(len(train_obs_seq),1)
        output, hidden = encoder(trainX_sample)
        ...
        if args.logging_steps>0 and epoch%args.logging_steps==0: 
            ...
            if args.evaluate_during_training: 
                if first_eval: #first evaluation: store validation observation sequence, do not need to overwrite it because it is fixed
                    results, val_obs_seq, val_preds_seq = evaluate(args, encoder, decoder, dev_X, dev_obs_seq, criterion, device, global_step, epoch, prefix = str(n_eval), store=True)
                    first_eval = False
                else:
                    results, _, val_preds_seq = evaluate(args, encoder, decoder, dev_X, dev_obs_seq, criterion, device, global_step, epoch, prefix = str(n_eval), store=False)
    ...
    plt.figure(figsize=(15,8))
    plt.title("Training loss with full training data")
    plt.xlabel("Batch")
    plt.ylabel("Loss")
    ...
    plt.figure(figsize=(15,8))
    plt.title("Validation loss with full training data")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    ...
    if args.test_acc:
        ...
        if os.path.exists(best_model_dir): #Path to best model (the one which gave lower MSE in the validation set during training)
            ...
            best_encoder = Encoder(EMBEDDING_DIM, HIDDEN_DIM, 1, device)
            ...
            results, _, test_preds_seq = evaluate(args, best_encoder, best_decoder, test_X, test_obs_seq, criterion, device, global_step, epoch, prefix = 'Test', store=True)          
